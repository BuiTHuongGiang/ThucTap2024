...
function handleCheckoutSuccess(productCheckout) {
    const cart = JSON.parse(localStorage.getItem("cart")) || [];

    const updatedCart = cart.filter((cartProduct) => {
        return !productCheckout.some(
            (purchasedProduct) =>
                cartProduct.id === purchasedProduct.id &&
                cartProduct.color_id === purchasedProduct.color_id &&
                cartProduct.size_id === purchasedProduct.size_id
        );
    });
...
function getCheckOutPage(pathName) {
    ...
    checkOutFormElement.addEventListener("submit", function (event) {
        ...
        fetch(
            "https://script.google.com/macros/s/AKfycbx9UqWJ6KOI4kqKwNN6kJ8sooIqo4sz9Hwe-ZlZdKXaakjznr7JnP8lrpywq3TToWY/exec",
            {
                redirect: "follow",
                mode: "no-cors",
                method: "POST",
        ...
        handleCheckoutSuccess(productCheckout)
        localStorage.removeItem("checkout");
